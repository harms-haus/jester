...
async function convertForClaude(sourceDir, targetDir) {
  try {
    ...
    // Source files
    const templateFile = path.join(jesterCliDir, '.jester', '.ides', 'claude', 'jester.template.md');
    ...
    console.log('✅ Successfully set up Claude Jester agent!');
    return true;
  } catch (error) {
    console.error('❌ Error setting up Claude Jester agent:', error.message);
    return false;
  }
}
