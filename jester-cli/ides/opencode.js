...
/**
 * Convert files for OpenCode IDE
 ...
 */
async function convertForOpenCode(sourceDir, targetDir) {
  try {
    ...
    // Source files
    const templateFile = path.join(jesterCliDir, '.jester', '.ides', 'opencode', 'jester.template.md');
    const jesterFile = path.join(jesterCliDir, '.jester', 'jester.md');

    // Destination file
    const destDir = path.join(projectRoot, '.opencode', 'agent');
    const destFile = path.join(destDir, 'jester.md');
    ...
    console.log('✅ Successfully set up OpenCode Jester rule!');
    return true;
  } catch (error) {
    console.error('❌ Error setting up OpenCode Jester rule:', error.message);
    return false;
  }
}
...
if (require.main === module) {
  convertForOpenCode().then(success => {
