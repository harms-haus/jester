...
2. **Basic agent files are created** for `/muse`, `/write`, and `/edit` commands as markdown files with YAML configuration following BMAD format
3. **Prompt-based agent system is functional** with basic slash command recognition and agent response routing
4. **File pipeline structure is established** with placeholder files for context.yaml, outline.md, and story.md
5. **Basic error handling is implemented** through agent responses for invalid commands and missing files
...
This story establishes the foundation for **prompt-based agents** rather than TypeScript classes. The agent framework will use:

- **Agent Definition Files**: Markdown files with YAML configuration in `.jester/agents/`
- **Prompt-Based Behavior**: Agent responses defined through structured prompts and rules
- **File-Based Communication**: Agents communicate through file operations, not direct code calls
- **Minimal Dependencies**: Only basic file utilities and LightRAG MCP client
