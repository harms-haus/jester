...
- [ ] Task 8: Implement Story Progression Validation Integration (AC: 8, 9)
  - [ ] 8.1. Create validation prompts for entity quality during draft → ready progression
    - [ ] 8.1.1. Create entity completeness validation prompt template
    - [ ] 8.1.2. Create entity quality assessment prompt template
    - [ ] 8.1.3. Create entity relationship validation prompt template
    - [ ] 8.1.4. Create entity metadata validation prompt template
  - [ ] 8.2. Implement entity formatting validation for ready → published progression
    - [ ] 8.2.1. Create entity file structure validation prompt
    - [ ] 8.2.2. Create entity markdown formatting validation prompt
    - [ ] 8.2.3. Create entity link syntax validation prompt
    - [ ] 8.2.4. Create entity patch formatting validation prompt
  - [ ] 8.3. Add validation templates for entity consistency checking
    - [ ] 8.3.1. Create entity consistency validation checklist template
    - [ ] 8.3.2. Create entity validation workflow prompt template
    - [ ] 8.3.3. Create entity validation result formatting template
    - [ ] 8.3.4. Create entity validation error handling template
  - [ ] 8.4. Create conflict detection for entity file operations
    - [ ] 8.4.1. Create entity file conflict detection prompt
    - [ ] 8.4.2. Create entity directory conflict scanning prompt
    - [ ] 8.4.3. Create entity conflict reporting prompt template
    - [ ] 8.4.4. Create entity conflict resolution prompt template
  - [ ] 8.5. Implement user approval workflows for entity validation
    - [ ] 8.5.1. Create entity validation approval request prompt template
    - [ ] 8.5.2. Create entity validation confirmation prompt template
    - [ ] 8.5.3. Create entity validation rejection handling prompt template
    - [ ] 8.5.4. Create entity validation workflow state management prompt
...
**NEW**: This story now includes comprehensive entity validation integration for story progression. The validation framework must be implemented through prompt-based agents following the existing architecture pattern. Key validation requirements:

- **Entity Quality Validation**: Ensure entity completeness and quality before draft → ready progression
- **Entity Formatting Validation**: Validate entity file structure and formatting before ready → published progression
- **Entity Consistency Checking**: Verify entity relationships and metadata consistency across progression stages
- **Entity Conflict Detection**: Identify and report entity file conflicts requiring user approval
- **User Approval Workflows**: Implement approval request and confirmation prompts for entity validation decisions
