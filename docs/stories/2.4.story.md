...
I want **to use a clear draft/ready/complete workflow for organizing my work**,
so that **I can easily manage work in progress, approved content, and published stories**.

## Acceptance Criteria

1. **Draft directory structure** is created with draft/, ready/, and complete/ folders with proper subdirectories
...
4. **Entity creation** happens directly in ready/ directory when approved, using standard naming conventions
...
10. **Entity file naming** follows consistent patterns: `{entity-name-hyphen-case}.md` for new entities, `{entity-name-hyphen-case}.patch.md` for patches
...
15. **Complete cleanup** removes all published files from ready/ directory after successful publish
16. **Change history** updates complete/ entity files with patch information and deletes patch files after application
...
**Draft Stage** (`draft/` directory):
- Work in progress with incrementing draft numbers
- Files: context-{number}.md, outline-{number}.md, story-{number}.md
- No entity folders - entities created directly in ready/ when approved
- No index on current files, incrementing numbers for history

**Ready Stage** (`ready/` directory):
- Approved work ready for publication
- Mirrors complete/ structure: stories/, outlines/, characters/, locations/, items/
- Clean naming without draft numbers

**Complete Stage** (`complete/` directory):
...
- `/edit approve-draft {draft-number}` - Move draft to ready/ directory
- `/edit publish "{story-title}"` - Move ready story to complete/ directory
- `/edit create-draft {draft-number}` - Create new draft with specified number
- `/edit list-drafts` - List all current drafts
- `/edit list-ready` - List all ready content
- `/edit list-complete` - List all complete content
...
- `ready/` - New directory for approved content
- `complete/` - New directory for published content
...
- Integration tests for draft → ready → complete progression
...
- Create draft/, ready/, complete/ directories
...
- Entities: Created directly in ready/ directory when approved
- Ready/Complete: Clean names without numbers

**Workflow Commands**:
- Draft creation and management
- Approval process (draft → ready)
- Publication process (ready → complete)
...
- [ ] 1.2. Create ready/ directory mirroring complete/ structure
- [ ] 1.3. Create complete/ directory for published content
...
- [ ] 3.4. Create entity creation workflow for ready/ directory
...
- [ ] 4.5. Create /edit list-ready and /edit list-complete commands
...
- [ ] 8.2. Test draft → ready → complete progression
...
- [ ] Draft/ready/complete directory structure created
...
- `ready/` - Ready content directory
- `complete/` - Complete content directory
...
## Change Log
- 2025-01-27: Story created to implement draft/ready/complete workflow system
...
- 2025-01-27: **COMPLETED** - Entity File Pipeline Implementation (Task 9)
  ...
  - Patch files located in entity directories (ready/{type}s/{entity-name}.patch.md)
...
- [x] Patch files located in entity directories (ready/{type}s/{entity-name}.patch.md)
...
**Files Verified:**
- `ready/characters/`, `ready/locations/`, `ready/items/` - Entity directories with patch files
- `complete/` - Target directory structure for published content with change history
